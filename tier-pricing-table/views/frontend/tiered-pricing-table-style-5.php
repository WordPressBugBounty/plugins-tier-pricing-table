...
<?php if ( ! empty( $pricing_rule->getRules() ) ) : ?>
	...
	<div class="tiered-pricing-wrapper">
		...
		<div class="tiered-pricing-table tiered-pricing-table--styled tiered-pricing-table--style-5 <?php echo ( isset( $settings['compact_layout'] ) && $settings['compact_layout'] === 'yes' ) ? 'tiered-pricing-table--slim' : ''; ?>"
		...
		>
			...
			<div class="tiered-pricing-table-body">
					...
					<?php if ( $settings['discount_column_title'] ) : ?>
						...
						<div>
							<?php if ( $discountAmount > 0 ) : ?>
								<span class="tiered-pricing-table-row-saving">
									<span class="tiered-pricing-table-row-bar"><i style="width: <?php echo esc_attr( $maxDiscount > 0 ? min( 100, round( $discountAmount / $maxDiscount * 100 ) ) : 0 ); ?>%"></i></span>
							...
							<?php else : ?>
								<span class="tiered-pricing-table-row-saving">
									<span class="tiered-pricing-table-row-bar"><i style="width: 0"></i></span>
				...
				<?php do_action( 'tiered_pricing_table/tiered_pricing/rows', $pricing_rule, $settings, 'tiered-pricing-table-style-5' ); ?>
				<?php do_action( 'tiered_pricing_table/table/tfoot', $pricing_rule, $settings, 'tiered-pricing-table-style-5' ); ?>
			</div>
		</div>
	...
	<style>
		...
		<?php echo esc_attr( '#' . $id ); ?> .tiered-pricing--active {
			background-color: <?php echo esc_attr( Settings::hex2rgba( $settings['active_tier_color'], 0.08 ) ); ?> !important;
			border-left-color: <?php echo esc_attr( 'no' === ( $settings['active_tier_border'] ?? 'yes' ) ? 'transparent' : $settings['active_tier_color'] ); ?> !important;
		}

		<?php $barColor = ! empty( $settings['discount_badge_color'] ) ? $settings['discount_badge_color'] : $settings['active_tier_color']; ?>
		<?php echo esc_attr( '#' . $id ); ?> .tiered-pricing-table-row-bar i {
			background-color: <?php echo esc_attr( Settings::hex2rgba( $barColor, 0.45 ) ); ?>;
		}

		<?php echo esc_attr( '#' . $id ); ?> .tiered-pricing--active .tiered-pricing-table-row-bar i,
		<?php echo esc_attr( '#' . $id ); ?> .tiered-pricing-table-row:hover .tiered-pricing-table-row-bar i {
			background-color: <?php echo esc_attr( $barColor ); ?>;
		}

		<?php echo esc_attr( '#' . $id ); ?> .tiered-pricing-table-row-discount:not(.tiered-pricing-table-row-discount--empty) {
			color: <?php echo esc_attr( $barColor ); ?>;
		}
