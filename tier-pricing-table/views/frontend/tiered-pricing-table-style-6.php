...
<?php if ( ! empty( $pricing_rule->getRules() ) ) : ?>
	...
	<div class="tiered-pricing-wrapper">
		...
		<?php
			$columns = 0;
			if ( $settings['quantity_column_title'] ) {
				$columns ++;
			}
			if ( $settings['discount_column_title'] ) {
				$columns ++;
			}
			if ( $settings['price_column_title'] ) {
				$columns ++;
			}
		...
		<div class="tiered-pricing-table tiered-pricing-table--styled tiered-pricing-table--style-6 <?php echo ( isset( $settings['compact_layout'] ) && $settings['compact_layout'] === 'yes' ) ? 'tiered-pricing-table--slim' : ''; ?>"
		...
		>
			<div class="tiered-pricing-table-header"
			...
			<div class="tiered-pricing-table-body">
				...
				<?php while ( $iterator->valid() ) : ob_start(); ?>
					...
					<div class="tiered-pricing-table-row"
						...
						<?php
							ob_start();
							do_action( 'tiered_pricing_table/tiered_pricing/row_columns', $pricing_rule,
									$currentQuantity );
							$custom_cols = ob_get_clean();
							echo wp_kses_post( str_replace( [ '<td', '</td>' ], [ '<div', '</div>' ], $custom_cols ) );
						?>
				...
				<?php do_action( 'tiered_pricing_table/tiered_pricing/rows', $pricing_rule, $settings, 'tiered-pricing-table-style-6' ); ?>

				<?php do_action( 'tiered_pricing_table/table/tfoot', $pricing_rule, $settings,
						'tiered-pricing-table-style-6' ); ?>
			</div>
		</div>
	...
	<style>
		...
		.tiered-pricing-table-row:last-child {
		...
		if ( $settings['clickable_rows'] && tpt_fs()->can_use_premium_code()) {
			...
			echo esc_attr('#' . $id) . ' .tiered-pricing-table-row:not(.tiered-pricing--active):hover { background-color: #f3f4f6; }';
		}
		?>
		...
		<?php if ( ! empty( $settings['discount_badge_color'] ) ) : ?>
		<?php echo esc_attr( '#' . $id ); ?> .tiered-pricing-table-row-discount {
			background-color: <?php echo esc_attr( Settings::hex2rgba( $settings['discount_badge_color'], 0.1 ) ); ?>;
			color: <?php echo esc_attr( $settings['discount_badge_color'] ); ?>;
		}
		<?php endif; ?>
	</style>
<?php endif; ?>
