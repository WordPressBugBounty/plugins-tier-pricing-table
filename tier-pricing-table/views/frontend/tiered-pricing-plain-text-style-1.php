...
<?php if ( ! empty( $price_rules ) ) : ?>
	...
	<div class="tiered-pricing-wrapper">
		...
		<ul class="tiered-pricing-plain-texts tiered-pricing-plain-texts--styled tiered-pricing-plain-texts--style-1"
			...
			>
				...
				<?php if ( $discountAmount > 0 ) : ?>
					...
					echo wp_kses_post( tptParsePlainText( $settings['plain_text_option_text'], $quantity,
							round( $discountAmount, 2 ), $price, $baseUnitName ) );
					?>
				<?php else : ?>
					<?php
	...
	<style>
		...
		echo esc_html( "#{$id} .tiered-pricing--active::before {
			background-color: {$settings['active_tier_color']};
