...
if ( !empty( $price_rules ) ) {
    ...
    ?>
		...
		<div class="tiered-pricing-options tiered-pricing-options--styled tiered-pricing-options--style-6 <?php 
    ...
    $tint = Settings::hex2rgba( $settings['active_tier_color'], 0.08 );
    $color = $settings['active_tier_color'];
    echo esc_html( "#{$id} .tiered-pricing--active { background: {$tint}; }" );
    echo esc_html( "#{$id} .tiered-pricing--active .tiered-pricing-option-checkbox { background: {$color}; border-color: {$color}; }" );
    echo esc_html( "#{$id} .tiered-pricing--active .tiered-pricing-option__discount-label { color: {$color}; }" );
    ?>
	</style>
<?php 
}