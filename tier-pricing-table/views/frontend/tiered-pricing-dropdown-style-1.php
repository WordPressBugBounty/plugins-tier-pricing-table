...
<?php if ( ! empty( $price_rules ) ) : ?>
	...
	<div class="tiered-pricing-wrapper">
		...
		<div class="tiered-pricing-dropdown tiered-pricing-dropdown--styled tiered-pricing-dropdown--style-1 <?php echo ( isset( $settings['compact_layout'] ) && $settings['compact_layout'] === 'yes' ) ? 'tiered-pricing-dropdown--slim' : ''; ?>"
				...
				<ul role="listbox" id="tiered-pricing-dropdown-list-<?php echo esc_attr( $id ); ?>">
					...
					<li class="tiered-pricing-dropdown-option tiered-pricing--active tiered-pricing-dropdown-option--selected tiered-pricing-dropdown-option--default"
						...
						<div class="tiered-pricing-dropdown-option__quantity">
							...
							<span class="tiered-pricing-dropdown-option__discount"><?php echo wp_kses_post( PricingTable::formatDiscount( $discountAmount, $pricing_rule, $product, null, $settings ) ); ?></span>
						<?php endif; ?>
					</div>
					...
					<?php while ( $iterator->valid() ) : ob_start(); ?>
						...
						<li class="tiered-pricing-dropdown-option"
							...
							<div class="tiered-pricing-dropdown-option__quantity">
								...
								?>
														<?php if ( $settings['show_discount_column'] && $discountAmount > 0 ) : ?>
								<span class="tiered-pricing-dropdown-option__discount"><?php echo wp_kses_post( PricingTable::formatDiscount( $discountAmount, $pricing_rule, $product, $currentQuantity, $settings ) ); ?></span>
							<?php endif; ?>
						</div>
...
EOD;
		...
		$pillCSS = <<<EOD
		#$id .tiered-pricing-dropdown__select-box {
			background: $backgroundColor;
		}
		#$id .tiered-pricing-dropdown-option__discount {
			background: {$settings['active_tier_color']};
		}
EOD;
		echo esc_html( $pillCSS );
